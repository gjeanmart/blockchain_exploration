...
(function(){
    ...
    function ethereumService ($log, $q, $filter, NETWORKS, commonService) {
        ...
        service.getTransactionReceipt = function (tx, gasPrice) {
            commonService.log.debug("ethereum.js", "getTransactionReceipt(tx="+tx+", gasPrice="+gasPrice+")", "START");
            
            return $q(function(resolve, reject)     {
                
               web3.eth.getTransactionReceipt(tx, function(err, result) {
                    if (err != null) {
                        commonService.log.error("ethereum.js", "getTransactionReceipt(tx="+tx+", gasPrice="+gasPrice+")", "END", "There was an error getting the transaction receipt : " + err);
					...
					commonService.log.debug("ethereum.js", "getTransactionReceipt(tx="+tx+", gasPrice="+gasPrice+")", "END", "transactionHash=" + receipt.transactionHash);
